...
import {deleteEmpleado, Empleado} from "@/app/api/APIEmpleados.jsx";
import usaLocalStorage from "@/hooks/usaLocalStorage.jsx";
import Link from "next/link";
import {useRouter, usePathname} from "next/navigation";
import {refreshNavigator} from "@/js/refreshNavigator.js";

const EmpleadoDelete = ({data, key}) => {
    const [id, setId] = useState(0);
    const [empleado, setEmpleado] = useState(data.Empleado);

    const pathname = usePathname();
    const router = useRouter();

    const delEmpleado = async (token) => {
         await deleteEmpleado(token, {
            id:empleado.id,
        })
        .then(response => {
        ...
        <div>
            ...
            <div className="p-4 md:p-5">
                <form className="space-y-10" onSubmit={handleUpdateClick}>
                    <label className="block mb-4 text-sm font-medium text-gray-900 dark:text-white">Se eliminará el
                        empleado
                        (<span className="text-green-700">{empleado.id}</span>) <span
                            className="text-orange-600">{empleado.full_name}</span>, desea continuar?
                    </label>
