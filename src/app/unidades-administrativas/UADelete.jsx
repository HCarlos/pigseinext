...
import {deleteUA, UA} from "@/app/api/APIUAs.jsx";
import usaLocalStorage from "@/hooks/usaLocalStorage.jsx";
import Link from "next/link";
import {useRouter, usePathname} from "next/navigation";

const UADelete = ({data, key}) => {
    const [id, setId] = useState(0);
    const [ua, setUA] = useState(data.UA);

    const pathname = usePathname();
    const router = useRouter();

    const delUA = async (token) => {
        await deleteUA(token, {
            id:ua.id,
        })
            .then(response => {
                console.log(response);
                location.href = "/unidades-administrativas";
            });
    }
        ...
        <div>
            ...
            <div className="p-4 md:p-5">
                <form className="space-y-10" onSubmit={handleUpdateClick}>
                    <label className="block mb-4 text-sm font-medium text-gray-900 dark:text-white">Se eliminará la Unidad Administrativa
                        (<span className="text-green-700">{ua.id}</span>) <span
                            className="text-orange-600">{ua.unidad}</span>, desea continuar?
                    </label>
