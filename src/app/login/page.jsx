...
function Login(callback, deps){
    ...
    const [helpassword, setHelpPassword] = useState('');
    ...
    const getTask = useCallback(async () => {
        ...
        try {
            const res = await fetch(`${process.env.NEXT_PUBLIC_API}api/v1/token/`, {
                method: "POST",
                body: JSON.stringify({username, password, recuerdame}),
                headers: {
            ...
            if (res.status === 200) {
                if (usaLocalStorage("_handleCheck", data.access, 0) !== "") {
                    usaLocalStorage("_handleCheck_u", username, 0)
                    usaLocalStorage("_handleCheck_rt", data.refresh, 0)
                    router.push(process.env.NEXT_PUBLIC_FRONT + "profile");
                }
            } else {
                setHelpUsername(data.username)
                setHelpPassword(data.password)
                setHelpHost(data.detail)
            }
    ...
    }, [password, recuerdame, router, username]);

    useEffect(() => {
        //getTask().then(r => {return ""});
    }, [getTask]);
                    ...
                    <div className="hidden w-full xl:block xl:w-1/2">
                        <div className="px-26 py-17.5 text-center">
                            ...
                            <p className="2xl:px-20">
                                Plataforma Integral de Gestión de Servicios de Equipos Informáticos.
                            </p>
                  ...
                  />
                    ...
                    <div className="w-full border-stroke dark:border-strokedark xl:w-1/2 xl:border-l-2">
                        <div className="w-full p-4 sm:p-12.5 xl:p-17.5">
                            <span className="mb-1.5 block font-medium">H. Ayutamiento de Centro, Tabasco, MX.</span>
                            <h2 className="mb-0 text-2xl font-bold text-black dark:text-white sm:text-title-xl2">
                                Ingresar al Sistema
                            </h2>
                            <span className="cursor-help mb-4 text-danger text-xs">{helphost}</span>
                            <form onSubmit={handleSubmit} className="mt-4">
                                <div className="mb-4">
                                    ...
                                    <div className="relative">
                                        <input
                                            type="text"
                                            name="username"
                                            placeholder="Escribe tu Nombre de Usuariol"
                                    ...
                                    <span className="cursor-help text-danger text-xs">{helpusername}</span>
                                </div>
                                ...
                                <div className="mb-6">
                                    ...
                                    <span className="cursor-help text-danger text-xs">{helpassword}</span>
                                </div>
